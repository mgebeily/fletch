...
const ChildComponent = ({ renderCounter }: { renderCounter: Function }) => {
  ...
  return <>
    <h1>{ name || 'Default' }</h1>
    <button onClick={() => commit('/name', 'Another name')}>
      Click me.
    </button>
  </>
}
...
const ParentComponent = ({ parentRenderCounter, childRenderCounter, adjacentRenderCounter, otherChildRenderCounter }: any) => {
  return <FletchContext>
    <NonUpdatingParentComponent renderCounter={parentRenderCounter}>
      <ChildComponent renderCounter={childRenderCounter}></ChildComponent>
      <OtherChildComponent renderCounter={otherChildRenderCounter}></OtherChildComponent>
      <NonUpdatingChildComponent renderCounter={adjacentRenderCounter}></NonUpdatingChildComponent>
...
describe('FletchContext', () => {
  it('updates on commit', async () => {
    ...
    const { findAllByText, getByText } = render(<ParentComponent otherChildRenderCounter={otherChildRenderCounter}
      parentRenderCounter={parentRenderCounter} childRenderCounter={childRenderCounter} adjacentRenderCounter={adjacentRenderCounter} />)
